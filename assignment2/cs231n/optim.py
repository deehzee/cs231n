...
weights and produces the next set of weights. Each update rule has the
same interface:
...
Inputs:
  ...
  - dw: A numpy array of the same shape as w giving the gradient of
    the loss with respect to w.
  - config: A dictionary containing hyperparameter values such as
    learning rate, momentum, etc. If the update rule requires caching
    values over many iterations, then config will also hold these
    cached values.
...
not perform well; however the default values of the other
...
def sgd_momentum(x, dx, config=None):
    ...
    # value in the next_x variable. You should also use and update   #
    # the velocity v.                                                #
    ##################################################################
...
def rmsprop(x, dx, config=None):
    ...
    squared gradient values to set adaptive per-parameter learning
    rates.
    ...
    next_x = None
    ##################################################################
    # TODO: Implement the RMSprop update formula, storing the next   #
    # value of x in the next_x variable. Don't forget to update      #
    # cache value stored in config['cache'].                         #
    ##################################################################
...
def adam(x, dx, config=None):
    ...
    next_x = None
    ##################################################################
    # TODO: Implement the Adam update formula, storing the next      #
    # value of x in the next_x variable. Don't forget to update the  #
    # m, v, and t variables stored in config.                        #
    ##################################################################
    learning_rate = config['learning_rate']
    beta1 = config['beta1']
    beta2 = config['beta2']
