...
def affine_forward(x, w, b):
    ...
    D = d_1 * ... * d_k, and then transform it to an output vector of
    dimension M.
    ...
    out = None
    ##################################################################
    # TODO: Implement the affine forward pass. Store the result in   #
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    incoming data. During training we also keep an exponentially
    decaying running mean of the mean and variance of each feature,
    and these averages are used to normalize data at test-time.

    At each timestep we update the running averages for mean and
    variance using an exponential decay based on the momentum
    parameter:
    ...
    time behavior: they compute sample mean and variance for each
    feature using a large number of training images rather than using
    a running average. For this implementation we have chosen to use
    running averages instead since they do not require an additional
    estimation step; the torch7 implementation of batch normalization
    ...
    if mode == 'train':
        ...
        # normalization. Use minibatch statistics to compute the     #
        # mean and variance, use these statistics to normalize the   #
        # incoming data, and scale and shift the normalized data     #
        # using gamma and beta.                                      #
        #                                                            #
        # You should store the output in the variable out. Any       #
        # intermediates that you need for the backward pass should   #
        # be stored in the cache variable.                           #
        #                                                            #
        # You should also use your computed sample mean and variance #
        # together with the momentum variable to update the running  #
        # mean and running variance, storing your result in the      #
        # running_mean and running_var variables.                    #
        ##############################################################
        ...
        cache = (x, gamma, beta, mu, sigma, x_cent, x_norm)
        ...
        ##############################################################
        # TODO: Implement the test-time forward pass for batch       #
        # normalization. Use the running mean and variance to        #
        # normalize the incoming data, then scale and shift the      #
        # normalized data using gamma and beta. Store the result in  #
        # the out variable.                                          #
        ##############################################################
        mu = running_mean
        ...
        raise ValueError(
            'Invalid forward batchnorm mode {}'.format(mode))
...
def batchnorm_backward(dout, cache):
    ...
    ##################################################################
    x, gamma, beta, mu, sigma, x_cent, x_norm = cache
    N, D = dout.shape

    # To follow this computation below, you need to see the
    # accompanying computation graph.
    ...
    # x_norm = x_cent / sigma
    # dx_norm -> dL/dx_norm [N x D]
    dx_norm = gamma * dout

    # dsigma -> dL/dsigma [D]
    dsigma = np.sum(-1. / (sigma ** 2) * x_cent * dx_norm, axis=0)

    # x_cent = x - mu
    # dx_cent -> dL/dx_cent [N x D]
    dx_cent = 1. / sigma * dx_norm
    ...
    # mu = mean(x)
    # dmu -> dL/dmu  [D] => N x D
    dmu = -np.sum(dx_cent, axis=0) + 2 * mu * dmu2

    # dx -> dL/dx  [N x D]
    dx = dx_cent + 2. / N * x * dmoment2 + 1. / N * dmu
    ##################################################################
    #                        END OF YOUR CODE                        #
    ##################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    For this implementation you should work out the derivatives for
    the batch normalizaton backward pass on paper and simplify as much
    as possible. You should be able to derive a simple expression for
    the backward pass.

    Note: This implementation should expect to receive the same cache
    variable as batchnorm_backward, but might not use all of the
    values in the cache.
    ...
    # inputs, you should be able to compute gradients with respect   #
    # to the inputs in a single statement; our implementation fits   #
    # on a single 80-character line.                                 #
    ##################################################################
    x, gamma, beta, mu, sigma, x_cent, x_norm = cache
    N, D = dout.shape
...
def dropout_forward(x, dropout_param):
    ...
    - dropout_param: A dictionary with the following keys:
      ...
      - mode: 'test' or 'train'. If the mode is train, then perform
        dropout; if the mode is test, then just return the input.
      - seed: Seed for the random number generator. Passing seed makes
        this function deterministic, which is needed for gradient
        checking but not in real networks.

    Outputs:
    - out: Array of the same shape as x.
    - cache: A tuple (dropout_param, mask). In training mode, mask is
      the dropout mask that was used to multiply the input; in test
      mode, mask is None.
    """
...
def dropout_backward(dout, cache):
    ...
    if mode == 'train':
        ##############################################################
        # TODO: Implement the training phase backward pass for       #
        # inverted dropout.                                          #
        ##############################################################
        dx = mask * dout
        ##############################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    The input consists of N data points, each with C channels, height
    H and width
    ...
    - out: Output data, of shape (N, F, H', W') where H' and W' are
      given by H' = 1 + (H + 2 * pad - HH) / stride W'
                  = 1 + (W + 2 * pad - WW) / stride
...
def max_pool_backward_naive(dout, cache):
    """
    A naive implementation of the backward pass for a max pooling
    layer.
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    - bn_param: Dictionary with the following keys:
      ...
      - momentum: Constant for running mean / variance. momentum=0
        ...
        time step, while momentum=1 means that new information is
        never incorporated. The default of momentum=0.9 should work
    ...
    # implementation should be very short; ours is less than five    #
    # lines.                                                         #
    ##################################################################
    pass
    ##################################################################
...
def svm_loss(x, y):
    ...
    margins = np.maximum(
        0, x - correct_class_scores[:, np.newaxis] + 1.0)
