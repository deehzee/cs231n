...
class KNearestNeighbor(object):
    ...
    def train(self, X, y):
        ...
        - X: A numpy array of shape (num_train, D) containing the
            training data consisting of num_train samples each of
            dimension D.
    ...
    def predict(self, X, k=1, num_loops=0):
        ...
        - k: The number of nearest neighbors that vote for the
            predicted labels.
        ...
        - y: A numpy array of shape (num_test,) containing predicted
            labels for the test data, where y[i] is the predicted
        ...
        if num_loops == 0:
            ...
            raise ValueError(
                'Invalid value {} for num_loops'.format(num_loops))
    ...
    def compute_distances_two_loops(self, X):
        ...
        Inputs:
        - X: A numpy array of shape (num_test, D) containing test
            data.
        ...
        for i in xrange(num_test):
            for j in xrange(num_train):
                ######################################################
                # TODO:                                              #
                # Compute the l2 distance between the ith test point #
                # and the j-th training point, and store the result  #
                # in dists[i, j]. You should not use a loop over     #
                # dimension.                                         #
                ######################################################
                dists[i, j] = np.sum(
    ...
    def compute_distances_one_loop(self, X):
        """
        Compute the distance between each test point in X and each
        training point in self.X_train using a single loop over the
        test data.
        ...
        for i in xrange(num_test):
            ...
            # all training points, and store the result in           #
            # dists[i, :].                                           #
            ##########################################################
            dists[i, :] = np.sum(
    ...
    def compute_distances_no_loops(self, X):
        ...
        # training points without using any explicit loops, and      #
        # store the result in dists.                                 #
        #                                                            #
        # You should implement this function using only basic array  #
        # operations; in particular you should not use functions     #
        # from scipy.                                                #
        #                                                            #
    ...
    def predict_labels(self, dists, k=1):
        ...
        Inputs:
        - dists: A numpy array of shape (num_test, num_train) where
            dists[i, j] gives the distance betwen the ith test point
        ...
        #                           END OF YOUR CODE                            #
        #######################################################################
        closest_ys = self.y_train[np.argsort(dists, axis=1)[:, :k]]
        for i in xrange(num_test):
            y_pred[i] = np.bincount(closest_ys[i]).argmax()
        return y_pred
